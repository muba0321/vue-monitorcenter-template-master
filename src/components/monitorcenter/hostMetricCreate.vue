...
import {sys_metric_host_create} from "@/api/monitorcenter/metrics";
  ...
  methods: {
    ...
    fetchHosts() {
      ...
      if (this.sysInfo) {
        ...
        fetchHostsBySysId(sys_id).then((response) => {
          this.list = response.data
    ...
    async handleAddRelation() {
      ...
      try {
        // 使用新的API方法
        const response = await sys_metric_host_create(postData);
